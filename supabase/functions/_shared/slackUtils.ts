...
const THREAD_EXPIRATION_HOURS = 24; // スレッドを有効とみなす時間（例: 24時間）
...
export async function getActiveThreadTs(chatId: string): Promise<string | null> {
  ...
  const expirationTime = new Date(Date.now() - THREAD_EXPIRATION_HOURS * 60 * 60 * 1000).toISOString();
  ...
  try {
    const { data, error } = await supabase
      ...
      .gt('last_updated_at', expirationTime)
...
/**
 * Saves or updates the thread timestamp for a given chat ID in the database.
 * @param chatId The Channel.io chat ID.
...
export async function saveThreadTs(chatId: string, threadTs: string): Promise<void> {
  ...
  try {
    const { error } = await supabase
      .from('slack_thread_store')
      .upsert(
        { channelio_chat_id: chatId, slack_thread_ts: threadTs, last_updated_at: now },
        { onConflict: 'channelio_chat_id' }
      );
